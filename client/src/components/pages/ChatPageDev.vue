    ...
    <div v-show="!isShowModal" class="chat-window">
      <div class="messages-container" ref="messagesContainer" @scroll="handleScroll">
        <ul class="message-list">
          ...
          <!-- <button v-show="isShowScrollBtn" class="chat-scrolltobottom-btn" ref="scrollToBottomBtn"></button> -->
          <ScrollToBottomBtn :top="temp"/>
        </ul>
      </div>
...
<script setup>
/* eslint-disable */
  import ScrollToBottomBtn from '@/components/ScrollToBottomBtn.vue';
  ...
  const handleScroll = () => {
    ...
    const btnElement = scrollToBottomBtn.value;
    ...
    // btnElement.style.bottom = `${container.scrollHeight - container.scrollTop - container.clientHeight}px`;

    // если уже почти доскролили, то прячем кнопку
    if(container.scrollHeight - container.scrollTop - container.clientHeight < 20) {
      isShowScrollBtn.value = false;
    } else {
      temp.value = container.scrollTop;
      // btnElement.style.bottom = `${container.scrollTop}px`;
    }

    // const scrollBottom = container.scrollHeight - container.scrollTop - container.clientHeight;

    // if (scrollBottom <= 0) {
    //   // Контейнер полностью прокручен вниз, скрываем кнопку
    //   isShowScrollBtn.value = false;
    // } else {
    //   // Контейнер не полностью прокручен вниз, показываем кнопку и устанавливаем bottom
    //   isShowScrollBtn.value = true;
    //   const btnElement = scrollToBottomBtn.value;
    //   if (btnElement) {
    //     console.log('scrollBottom: ', scrollBottom)
    //     btnElement.style.bottom = `${scrollBottom}px`;
    //   }
    // }
  }
